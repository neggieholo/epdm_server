...
const { IndustryChallenge } = require('../../models/admin');
...
router.post("/create", upload.array("media"), async (req, res) => {
    ...
    try {
        ...
        // Save
        const saved = new IndustryChallenge({ heading, sections: parsedSections });
        await saved.save();

        res.json({ success: true, industryChallenge: saved });
    } catch (err) {
...
router.get("/", async (req, res) => {
    try {
        const all = await IndustryChallenge.find().sort({ createdAt: -1 });
        res.json({ success: true, industrychallenges: all });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.get("/:id", async (req, res) => {
    try {
        const industryChallenge = await IndustryChallenge.findById(req.params.id);
        if (!industryChallenge) {
            return res.status(404).json({ error: "Industry challenges not found" });
        }

        res.json({ success: true, industryChallenge });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.delete("/delete/:id", async (req, res) => {
    ...
    try {
        await IndustryChallenge.findOneAndDelete({ _id: req.params.id });
